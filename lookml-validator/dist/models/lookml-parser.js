    ...
    customParseDocument(document, result) {
        ...
        if (result.isModelFile && result.modelName) {
            const modelInfo = {
                ...
                location: vscode_languageserver_types_1.Location.create(uri, vscode_languageserver_types_1.Range.create(vscode_languageserver_types_1.Position.create(0, 0), vscode_languageserver_types_1.Position.create(0, 0))),
                includes: [],
                explores: [],
                properties: new Map(),
            };
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (result.isModelFile && topLevelPropertyMatch && blockStack.length === 0) {
                ...
                const location = vscode_languageserver_types_1.Location.create(uri, vscode_languageserver_types_1.Range.create(vscode_languageserver_types_1.Position.create(i, 0), vscode_languageserver_types_1.Position.create(i, lines[i].length)));
                const propertyInfo = {
                    name: propertyName,
                    value: propertyValue,
                    location: location,
