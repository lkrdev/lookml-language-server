...
export class PropertyCompletionProvider {
  ...
  private readonly commonProperties: { [key: string]: string[] } = {
    view: [
      "sql_table_name", "extends", "derived_table", "label", 
      "description", "drill_fields", "required_access_grants"
    ],
    dimension: [
      "type", "sql", "label", "group_label", "description", "hidden", 
      "primary_key", "value_format", "html", "required_access_grants"
    ],
    dimension_group: [
      "type", "sql", "timeframes", "label", "description", "hidden",
      "convert_tz", "datatype"
    ],
    measure: [
      "type", "sql", "label", "group_label", "description", "hidden", 
      "value_format", "drill_fields", "html", "filters"
    ],
    explore: [
      "view_name", "from", "join", "label", "description", "hidden",
      "always_filter", "conditionally_filter", "fields"
    ],
    join: [
      "type", "relationship", "sql_on", "from", "view_name", "fields",
      "required_joins"
    ],
    parameter: [
      "type", "default_value", "allowed_values", 
      "label", "description", "hidden"
    ],
    filter: [
      "type", "sql", "default_value", 
      "label", "description", "hidden"
    ],
    model: [
      "connection", "include", "fiscal_month_offset", "week_start_day", 
      "case_sensitive", "explore"
    ]
  };
  
  // Type completions for different block types
  private readonly dimensionTypes = [
    "string", "number", "date", "time", "datetime", "yesno", 
    "tier", "zipcode"
  ];
  
  private readonly measureTypes = [
    "count", "count_distinct", "sum", "average", "max", "min", 
    "median", "number"
  ];
  ...
  // Join types
  private readonly joinTypes = [
    "left_outer", "full_outer", "inner", "cross"
  ];
  ...
  public getCompletions(context: CompletionContext): CompletionItem[] {
    if (context.type === 'property') {
      ...
      return this.getPropertyValueCompletions(context.propertyName, context.blockType);
    }
   ...
   */
  private getPropertyCompletions(blockType: string | undefined): CompletionItem[] {
  ...
  private getTypeCompletions(blockType: string | undefined): CompletionItem[] {
    if (blockType === "dimension" || blockType === "filter" || blockType === "parameter") {
  ...
  private getPropertyValueCompletions(
    ...
    switch (propertyName) {
      ...
      case "timeframes":
        return ["raw", "time", "date", "week", "month", "quarter", "year"].map(value => ({
          label: value,
          kind: CompletionItemKind.EnumMember,
          data: { type: "timeframe" }
        }));
