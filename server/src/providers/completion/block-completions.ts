...
export class BlockCompletionProvider {
   ...
   */
  private getBlockNameCompletions(blockType: string | undefined): CompletionItem[] {
    if (!blockType) return [];
  ...
  private hasSnippet(blockType: string): boolean {
    return ["view", "explore", "dimension", "dimension_group", "measure", "join", "filter"].includes(blockType);
  }
}