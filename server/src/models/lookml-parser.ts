...
export class LookMLParser {
  ...
  /**
   * Get the block at a given position in the document
   */
  public getBlockAtPosition(document: TextDocument, position: Position): { properties: { [key: string]: any } } | undefined {
    const text = document.getText();
    const lines = text.split("\n");
    const blockStack: { type: string; name: string; level: number; startLine: number; properties: { [key: string]: any } }[] = [];

    // Scan backwards from current line to find the current block
    for (let i = position.line; i >= 0; i--) {
      ...
      if (line.match(/^\s*\}/)) {
        // Remove blocks that have ended
        while (blockStack.length > 0 && blockStack[blockStack.length - 1].level >= lineIndent) {
          blockStack.pop();
        }
        continue;
      }
      ...
      if (blockMatch) {
        ...
        // Only add to stack if this block is a parent (has less indentation)
        if (blockStack.length === 0 || lineIndent < blockStack[blockStack.length - 1].level) {
          blockStack.push({
            type: blockType,
            name: blockName,
            level: lineIndent,
      ...
      if (propertyMatch && blockStack.length > 0) {
        ...
        const currentBlock = blockStack[blockStack.length - 1];
        currentBlock.properties[propertyName] = propertyValue;
      }
    }

    // Return the current block if we found one
