...
import { ExtendedLooker40SDK } from "./extended_sdk";
...
export class AuthenticationService {
    ...
    private sdk: ExtendedLooker40SDK | null = null;
    ...
    private setupHttpServer() {
        this.server = http.createServer(async (req, res) => {
            ...
            if (
                parsedUrl.pathname ===
                AuthenticationService.DEFAULT_CALLBACK_PATH
            ) {
                try {
                    ...
                    this.sdk = new ExtendedLooker40SDK(this.oauthSession);
    ...
    private stopServer() {
        if (this.server) {
            ...
            console.log(
                "OAuth callback server was not running or already stopped.",
            );
        }
    }
    ...
    public async initializeOAuth(): Promise<void> {
        ...
        if (existing) {
            ...
            const now_plus_24_hours = new Date(
    ...
    public async resetToRemote(
        ...
        try {
            ...
            if (result.ok) {
                return {
                    success: true,
                    message: "Successfully reset to remote",
                };
            } else {
                return {
                    success: false,
            ...
            console.error(
                `Failed to reset to remote (${project_name}):`,
                error,
            );
            return {
                success: false,
    ...
    public async validateProject(
        ...
        try {
            ...
            console.error(
                `Failed to validate project (${project_name}):`,
                error,
            );
            return {
                success: false,
    ...
    public async getSDK(
        db_auth_token?: AuthRecord,
    ): Promise<ExtendedLooker40SDK> {
